...
description: Per-turn token spend breakdown of the current Claude Code session. Fully code-driven — the MCP returns a pre-formatted visual report; the skill only prints it. Invoke when the user wants to understand the shape of their session's consumption.
---

# cf-session-consumption

1. Call `render_consumption_report` with no arguments (or `{ "full": true }` if the user explicitly asks for every turn).
2. Print the `text` field **verbatim**. Do not reformat, reinterpret, add analysis, or make other tool calls.
